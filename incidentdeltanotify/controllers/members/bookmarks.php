...
/**
 * Incident delta notify controller. Lets admins see the ideas they've bookmarked
...
class Bookmarks_Controller extends Members_Controller {

	public function __construct()
	{
		parent::__construct();

		$this->template->this_page = 'bookmarks';
	...
	public function index($page = 1)
	{
		...
		$this->template->content = new View('incidentdeltanotify/members/bookmarks');
		...
		// Fetch all incidents that this user is following
		$all_incidents = ORM::factory('incidentdeltanotify')
		...
		$limit = substr($pagination->sql_limit, 6, strpos($pagination->sql_limit, ' OFFSET')-6);

		// Reports
		//$incidents = Incident_Model::get_incidents(reports::$params, $pagination);
		$incidents = ORM::factory('incident')
			->join('incidentdeltanotify', 'incidentdeltanotify.incident_id', 'incident.id')
			->where('incidentdeltanotify.user_id', $this->user->id)
			->limit($limit)
