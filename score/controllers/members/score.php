...
/**
 * Score controller. Lets admins see the their score info
 * shamelessly stolen from application/controllers/admin/reports
 *
 * @author	   John Etherton <john@ethertontech.com> 
 * @package	   Score - http://ethertontech.com
 */

class Score_Controller extends Members_Controller {

	public function __construct()
	{
		parent::__construct();

		$this->template->this_page = 'Score';
	...
	public function index()
	{
		...
		$this->template->content = new View('score/members/myscore');
		$this->template->content->user = $this->user;
		$this->template->content->title = Kohana::lang('score.score_title');
		...
		//get the number of ideas this user has had
		$your_ideas = ORM::factory('incident')
			->where('user_id', $this->user->id)
			->where('incident_active', 1)
			->find_all();
		
		//now prep the SQL to get the number of votes for your ideas
		$ideas_ids = null;
		foreach($your_ideas as $idea)
		{
			if($ideas_ids != null)
			{
				$ideas_ids .= ",";
		...
		$your_ideas_votes = 0;
		$your_ideas_scores = array();		
		
		if($ideas_ids != null)
		{
			
			$your_ideas_votes = ORM::factory('score')
			->in('incident_id', $ideas_ids)
			->where('vote', 1)
			->count_all();
			
			
			//get the votes for your ideas
			$votes_for_your_ideas = ORM::factory('score')
			->select('incident_id, count(id) as num')
			->in('incident_id', $ideas_ids)
			->where('vote', 1)
			->groupby('incident_id')
			->find_all();
			
			//get the votes against  your ideas
			$votes_against_your_ideas = ORM::factory('score')
			->select('incident_id, count(id) as num')
			->in('incident_id', $ideas_ids)
			->where('vote', -1)
			->groupby('incident_id')
			...
			foreach($votes_for_your_ideas as $vfi)
			{
				$your_ideas_scores[$vfi->incident_id] = array('for'=>$vfi->num, 'against'=>0);
			}
			foreach($votes_against_your_ideas as $vai)
			{
				if(!isset($your_ideas_scores[$vai->incident_id]))
				{
					$your_ideas_scores[$vai->incident_id] = array('for'=>0, 'against'=>0);
				}
				$your_ideas_scores[$vai->incident_id]['against'] = $vai->num;
			}
			foreach($your_ideas as $ideas)
			{
				if(!isset($your_ideas_scores[$ideas->id]))
				{
					$your_ideas_scores[$ideas->id] = array('for'=>0, 'against'=>0);
				}
			}
		}
		
		$your_votes = ORM::factory('incident')
		->select('*, score.vote as vote')
		->join('score', 'score.incident_id', 'incident.id')
		->where('score.user_id', $this->user->id)
		->find_all();

		$this->template->content->your_ideas = $your_ideas;
		$this->template->content->your_ideas_votes = $your_ideas_votes;
		$this->template->content->your_votes = $your_votes;
		$this->template->content->your_ideas_scores = $your_ideas_scores;
