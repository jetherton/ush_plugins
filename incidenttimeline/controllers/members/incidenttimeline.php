...
class Incidenttimeline_Controller extends Members_Controller {
			...
			if($post->validate())
			{
				...
				switch ($post->save)
				{
					...
					case 'dontclose':
						...
						url::redirect('members/incidenttimeline/edit/'. $timeline->id .'/saved');
						break;
					case 'addnew':
						// Save and add new
						url::redirect('members/reports/edit/0/saved');
						break;
					default:
						// Save and close
						url::redirect('members/reports/edit/'.$incident->id);
				}
			}
	...
	function deletePhoto ( $id )
	{
		...
		if ( $id )
		{
			...
			$timeline->photo = null;
			$timeline->save();
			Event::run('incidenttimeline_action.timeline_edit',$id);
		}
	}
